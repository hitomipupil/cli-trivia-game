...
const trivias = [
  ...
  },
  {
    ...
    options: ['Port of Antwerp', 'Port of Hamburg', 'Port of Amsterdam', 'Port of Rotterdam'],
...
const questionText = (trivia) => {
  const optionsText = trivia.options.map((opt, index) => {
    return `${index + 1}: ${opt}`
  }).join('\n')
...
program
  ...
  .action(async() => {
    ...
    const result = choice === (preparedTrivias[i].correctChoice) ? "Correct!" : "Incorrect!"
    if(result === "Correct!"){
      score ++
    }
